...
class JsonRpcServer
{
    ...
    {
        return ($requestObject->jsonrpc == RpcResponse::VERSION && $this->isValidRequestObjectId($requestObject->id) && $this->isValidRequestObjectMethod($requestObject->method));
    }

    private function isValidRequestObjectId($requestId)
    {
        return (is_null($requestId) || is_string($requestId) ||
    ...
    {
        return (! is_null($requestMethod) && is_string($requestMethod) && strncmp($reserved = "rpc.", $requestMethod, strlen($reserved)));
    }
...
//         echo '<pre>';
//         var_dump($this->_listOfCallableServices[0]->getCallableMethodNames());
//         echo '</pre>';
//         exit;
    ...
    private function validateAndSortParameters($methodOwnerService, $requestObject)
    {
        ...
        if ($this->isValidParamsNumber($validParameters, $requestObject) && $this->isValidParamsName($validParameters, $requestObject)) {
    ...
    private function call($methodOwnerService, $requestObject)
    {
        ...
        return call_user_func_array($callbackFunction, is_string($requestObject->params)?json_decode($requestObject->params,true):$requestObject->params);
    }
